...
if __name__ == "__main__":    
    ...
    if cluster=='h': 
        OUT_PATH = '/n/dtak/jfutoma/prediction_constrained_RL/experiments/tiger/'
    if cluster=='d':
        OUT_PATH = '/hpchome/statdept/jdf38/prediction_constrained_RL/experiments/tiger/'
    if cluster=='l':
        OUT_PATH = '/Users/josephfutoma/Dropbox/research/prediction_constrained_RL/experiments/results/tiger/'

    RESULTS_FOLDER = 'results_3oct/'
    LOGS_FOLDER = 'logs_3oct/'
    ...
    hyperparams_all = itertools.product(seeds,lambds,inits,sig_goods,sig_others,n_envs,prune_nums,ESS_penalties,folds)
    ct = 0
    for hyperparams in hyperparams_all:
        if job_id == ct:
            seed,lambd,param_init,sig_good,sig_other,n_env,prune_num,ESS_penalty,fold = hyperparams
        ct += 1
    ...
    tiger_env = True
    lr = 1e-3
    model_string = '%s_nenv%d_siggood%.1f_sigother%.1f_N%d_lambd%.8f_init-%s_prune%d_ESSpenalty%.3f_seed%d_fold%d' %(
